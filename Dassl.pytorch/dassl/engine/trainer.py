...
from class_destroy import get_jigsaw_tensor
from prorandconv import pro_rand
...
class TrainerBase:
    ...
    def train(self, start_epoch, max_epoch):
        ...
        one_image = next(iter(self.test_loader))
        
        index = self.cfg.flag
        self.one_image = one_image['img'][index]
        self.one_label = one_image['label'][index]
...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):
          ...
          #destructed = get_jigsaw_tensor(input,(224,224),4) ##ここ加えた
          
          output = self.model_inference(input)
          
          #destructed_output = self.model_inference(destructed) #ここ加えた
          #output = F.relu(F.softmax(output,dim=1)-F.softmax(destructed_output,dim=1)) # ここ加えた
          ...
          min = 10
          for _ in range(N):
            weak_image = pro_rand(input, n_loop=5, r=0.5)
            output_weak = self.model(weak_image)
            entropy_weak = get_entropy(output_weak)
            if entropy_weak < min:
              min = entropy_weak
              weak_image_best = weak_image
          weak_image = weak_image_best
          weak_destruct = get_jigsaw_tensor(weak_image,(224,224),4)
          output = self.model_inference(weak_destruct)
          '''
          ###ここまで改造
...
class TrainerX(SimpleTrainer):
    ...
    def run_epoch(self):
        ...
        for self.batch_idx, batch in enumerate(self.train_loader_x):
            ...
            meet_freq = (self.batch_idx + 1) % self.cfg.TRAIN.PRINT_FREQ == 0
            only_few_batches = self.num_batches < self.cfg.TRAIN.PRINT_FREQ
            if meet_freq or only_few_batches or True:
                nb_remain = 0
                nb_remain += self.num_batches - self.batch_idx - 1
                nb_remain += (
                    self.max_epoch - self.epoch - 1
