...
def entropy_maximize(outputs):
  ### 既にsoftmaxしてある前提
  log_probabilities = torch.log(outputs + 1e-5) 
  entropy = torch.sum(outputs * log_probabilities, dim=1)
...
def entropy_for_destructed(output):
  ### softmaxせずに渡す
  prob = F.softmax(output,dim=1)
  entropy = torch.sum(prob * torch.log(prob + 1e-5), dim=1)
  return -entropy
...
class PromptLearner(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        #prompts = ["a " + prompt_prefix + " style of a " + name + "." for name in classnames]
...
class CoOp(TrainerX):
    ...
    def build_model(self):
        ...
        #classnames = ["bird","car","chair","dog","person"] # when using VLCS
    ...
    def forward_backward(self, batch):
        ...
        label[:] = self.one_label

        with torch.no_grad():
          #sigma = self.cfg.sigma
          resize = v2.Resize(size=224)
          vflip = v2.RandomVerticalFlip(p=1)
          blur = v2.GaussianBlur(kernel_size=(5,9), sigma=(10.,30.))
          grid = self.cfg.grid
          
          ####### #######     Class Destruction    ####### ####### 
          destructed_image = image
          destructed_image = blur(image)
          destructed_image = get_jigsaw_tensor(destructed_image, resize=(224,224), grid=grid)
          destructed_image = resize(destructed_image) ### Gridが増えるとこれが必要になる
          destructed_image = blur(destructed_image)
          destructed_image = vflip(destructed_image)
          destructed_image[:] = destructed_image[0]

        
        #torchvision.utils.save_image(image[0], "test.png")
        #time.sleep(1)
        ...
        if prec == "amp":
            with autocast():
              ...
              loss = -entropy_maximize(output)
            self.optim.zero_grad()
            self.scaler.scale(loss).backward()
            self.scaler.step(self.optim)
            self.scaler.update()
        else:
            output, output_local = self.model(destructed_image) ## output_local.shape = (batch_size, 14*14, # of classes)
            output_original, output_original_local = self.model(image)

            ##### entropy bottom k minimize
            
            k = self.cfg.K
            local_entropy = get_entropy_local(output_original_local)
            topk_index = torch.topk(local_entropy, k=k, dim=1)[1]
            bottomk_index = torch.topk(-local_entropy, k=k, dim=1)[1]
            loss = -get_entropy(output)
            
            for i in range(k):
              index = topk_index[0][i]
              #topk = topk_index[0][i]
              bottomk = bottomk_index[0][i]
              loss += get_entropy(torch.unsqueeze(output_original_local[0][bottomk],dim=0)) / 196 * weight
            ...
            #loss = -get_entropy(output)
            #local_entropy = get_entropy_local(output_original_local)
            #chosen_index = (local_entropy < threshold).nonzero()
            #for index in chosen_index:
            #    loss += get_entropy(torch.unsqueeze(output_original_local[0][index[1]],dim=0)) / 196 * weight
